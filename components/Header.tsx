...
export const Header: React.FC<HeaderProps> = ({ onShowMenu, onShowCart }) => {
    ...
    >
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <button 
              onClick={onShowCart}
              className="relative p-2 text-gray-600 hover:text-bakery-600 transition-colors"
            >
              <ShoppingBag className="w-5 h-5" />
              {totalItems > 0 && (
                <span className="absolute top-0 right-0 w-4 h-4 bg-accent-blue text-white text-[10px] flex items-center justify-center rounded-full font-bold">
                  {totalItems}
                </span>
              )}
            </button>
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 right-0 bg-white border-t shadow-lg md:hidden p-4 flex flex-col gap-4 animate-in slide-in-from-top-2">
          ...
          <Button variant="outline" className="w-full flex items-center justify-center gap-2" onClick={() => { setIsMobileMenuOpen(false); onShowCart(); }}>
            <ShoppingBag className="w-5 h-5" />
